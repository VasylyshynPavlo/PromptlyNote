...
namespace PromptlyNote.Core.Interfaces.Services
{
    public interface ICategoryService
    {
        ...
        Task<PagedResult<CategoryDto>> ListAsync(string userId, int page = PaginationConfiguration.MinimumPage, int pageSize = PaginationConfiguration.DefaultPageSize, CategorySortBy sortBy = CategorySortBy.Name, bool includeTasks = false, CancellationToken cancellationToken = default);
        Task<CategoryDto> GetAsync(string id, string userId, bool includeTasks = false, CancellationToken cancellationToken = default);
    }
}
